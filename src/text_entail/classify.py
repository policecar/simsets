...
def run_classification_test(mat, true_labels, binarize = True, percentage_train=0.8, print_train_test_set_stat = True, test_thresholds=False, random_seed=None, d_args=None):
...
def get_train_test_indexes_presplit(d_args):
...
def calculate_statistics(true_test_labels, predicted_test_labels):
    ...
    true_test_labels = true_test_labels.astype(bool);
    predicted_test_labels = predicted_test_labels.astype(bool);
    tp = true_test_labels & predicted_test_labels;
    fp = tp ^ predicted_test_labels;
    fn = tp ^ true_test_labels;
    
    sum_tp = tp.sum();
    sum_fp = fp.sum();
    sum_fn = fn.sum();
    sum_tn = true_test_labels.shape[0] - sum_tp - sum_fp - sum_fn;
    
    prec_1 = sum_tp / (sum_tp + sum_fp);
    rec_1  = sum_tp / (sum_tp + sum_fn);
    acc  = (sum_tp + sum_tn) / true_test_labels.shape[0];
    ...
    w1 = true_test_labels.sum() / len(true_test_labels);
    w2 = 1 - w1;
